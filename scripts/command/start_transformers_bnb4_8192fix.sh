...
RESULT_DIR="${PROJECT_DIR}/results/transformers_bnb4_nohup_8192fix"
...
if ss -ltnp 2>/dev/null | grep -q ":${PORT} "; then
  echo "端口 ${PORT} 已被占用，请先停止旧 Transformers BNB4 服务后再启动。"
  ss -ltnp 2>/dev/null | grep ":${PORT} " || true
  exit 1
fi

echo "启动 Transformers BNB4 服务，日志写入：${LOG_DIR}/transformers.log"
...
echo "Transformers BNB4 已提交后台启动，PID: $(cat "${RESULT_DIR}/transformers.pid")"
